...
class Calendario:
    ...
    def nota(self):
        ...
        application = index.Product(self.vent)
        tk.Label(self.vent, text = "Fecha: ").grid(row = 1, column = 0)
        self.vent.mainloop()

    def Imprimir_Cal(self, y, m):

        Mes_Anterior = tk.Button(self.Ventana,bg="lightgreen", font=("courier", 10, "bold"), text="<=", command=self.Mes_Anterior)
        self.wid.append(Mes_Anterior)
        Mes_Anterior.grid(row=8, column=1)

        Mes_Siguiente = tk.Button(self.Ventana,bg="lightgreen", font=("courier", 10, "bold"), text="=>", command=self.Mes_Siguiente)
        self.wid.append(Mes_Siguiente)
        Mes_Siguiente.grid(row=8, column=5)

        A_Anterior = tk.Button(self.Ventana,bg="lightgreen", font=("courier", 10, "bold"), text="<<=", command=self.A_Anterior)
        self.wid.append(A_Anterior)
        A_Anterior.grid(row=9, column=1)

        A_Siguiente = tk.Button(self.Ventana,bg="lightgreen", font=("courier", 10, "bold"), text="=>>", command=self.A_Siguiente)
        self.wid.append(A_Siguiente)
        A_Siguiente.grid(row=9, column=5)
        ...
        for w, week in enumerate(self.cal.monthdayscalendar(y, m), 2):
            for d, day in enumerate(week):
                if day:
                    ...
                    u="{0:02d}".format(day)
                    b = tk.Button(self.Ventana,bg="lightgreen", text=u, font=("courier", 24, "bold"), command= self.nota)
                    self.wid.append(b)
